...
bool cSoftBodyVerlet::CreateSoftBodyBridge(sModelDrawInfo ModelInfo, glm::mat4 matInitalTransform /*=glm::mat4(1.0f)*/ )
{
	// Copy the model info
	this->m_ModelVertexInfo = ModelInfo;
	// Copy the vertices and indices as well
...
//		pVerticesLocalCopy[index].w = ModelInfo.pVertices[index].w;
	...
	for ( unsigned int index = 0; index < this->m_ModelVertexInfo.numberOfIndices; index += 3 )
	{
		...
		if ((pParticle1->pModelVertex->x == pParticle2->pModelVertex->x) || (pParticle1->pModelVertex->z == pParticle2->pModelVertex->z))
		{
			sConstraint* pEdge1 = new sConstraint();
			pEdge1->pParticleA = pParticle1;
			pEdge1->pParticleB = pParticle2;
		...
		{
			std::cout << "p1: " << pParticle1->pModelVertex->x << ", " << pParticle1->pModelVertex->z <<
				" p2: " << pParticle2->pModelVertex->x << ", " << pParticle2->pModelVertex->z << std::endl;

		}

		if ((pParticle3->pModelVertex->x == pParticle2->pModelVertex->x) || (pParticle3->pModelVertex->z == pParticle2->pModelVertex->z))
		{
		...
		{
			std::cout << "p3: " << pParticle3->pModelVertex->x << ", " << pParticle3->pModelVertex->z <<
				" p2: " << pParticle2->pModelVertex->x << ", " << pParticle2->pModelVertex->z << std::endl;
		}

		if ((pParticle1->pModelVertex->x == pParticle3->pModelVertex->x) || (pParticle1->pModelVertex->z == pParticle3->pModelVertex->z))
		{
			sConstraint* pEdge3 = new sConstraint();
			pEdge3->pParticleA = pParticle3;
			pEdge3->pParticleB = pParticle1;
		...
		{
			std::cout << "p3: " << pParticle3->pModelVertex->x << ", " << pParticle3->pModelVertex->z <<
				" p1: " << pParticle1->pModelVertex->x << ", " << pParticle1->pModelVertex->z << std::endl;
...
void cSoftBodyVerlet::ApplyCollision(double deltaTime)
{
	...
	for (sParticle* pCurrentParticle : vec_pParticles)
	{
		...
		for (int i = 0; i < vec_pPlankPositions.size(); i++)
		{
			//std::cout << "x: " << vec_pPlankPositions[i].x << " y: " << vec_pPlankPositions[i].y << " z: " << vec_pPlankPositions[i].z << std::endl;
			if (vec_pPlankPositions[i].x != 0.0f)
			{
				//if (pCurrentParticle->position.y < theAABB->minXYZ.y + vec_pPlankPositions[i].y)
				//{
				//	//std::cout << "1" << theAABB->minXYZ.y + vec_pPlankPositions[i].y << " Y: " << pCurrentParticle->position.y << std::endl;
				//	pCurrentParticle->position.y = theAABB->minXYZ.y + vec_pPlankPositions[i].y;
				//}
				if (pCurrentParticle->position.x < theAABB->maxXYZ.x + vec_pPlankPositions[i].x && pCurrentParticle->position.x > theAABB->minXYZ.x + vec_pPlankPositions[i].x)
				{
					if (pCurrentParticle->position.z > theAABB->minXYZ.z + vec_pPlankPositions[i].z && pCurrentParticle->position.z < theAABB->maxXYZ.z + vec_pPlankPositions[i].z)
					{
						if (pCurrentParticle->position.y < theAABB->maxXYZ.y + vec_pPlankPositions[i].y && !(pCurrentParticle->position.y < vec_pPlankPositions[i].y))
						{
							//std::cout << "2" << theAABB->maxXYZ.y + vec_pPlankPositions[i].y << " Y: " << pCurrentParticle->position.y << std::endl;
							pCurrentParticle->position.y = theAABB->maxXYZ.y + vec_pPlankPositions[i].y;
						}
						else if (pCurrentParticle->position.y > theAABB->minXYZ.y + vec_pPlankPositions[i].y && !(pCurrentParticle->position.y > vec_pPlankPositions[i].y))
						{
							pCurrentParticle->position.y = theAABB->minXYZ.y + vec_pPlankPositions[i].y;
						}
					}
				...
				if (pCurrentParticle->position.y < theAABB->maxXYZ.y + vec_pPlankPositions[i].y && pCurrentParticle->position.y > theAABB->minXYZ.y + vec_pPlankPositions[i].y)
				{
					if (pCurrentParticle->position.z > theAABB->minXYZ.z + vec_pPlankPositions[i].z && pCurrentParticle->position.z < theAABB->maxXYZ.z + vec_pPlankPositions[i].z)
					{
						if (pCurrentParticle->position.x < theAABB->maxXYZ.x + vec_pPlankPositions[i].x && !(pCurrentParticle->position.x < vec_pPlankPositions[i].x))
						{
							//std::cout << "2" << theAABB->maxXYZ.x + vec_pPlankPositions[i].x << " Y: " << pCurrentParticle->position.x << std::endl;
							pCurrentParticle->position.x = theAABB->maxXYZ.x + vec_pPlankPositions[i].x;
							acceleration.x = 0.0f;
						}
						else if (pCurrentParticle->position.x > theAABB->minXYZ.x + vec_pPlankPositions[i].x && !(pCurrentParticle->position.x > vec_pPlankPositions[i].x))
						{
							pCurrentParticle->position.x = theAABB->minXYZ.x + vec_pPlankPositions[i].x;
							acceleration.x = 0.0f;
						}
					}
				}
				...
				if (pCurrentParticle->position.y < theAABB->maxXYZ.y + vec_pPlankPositions[i].y && pCurrentParticle->position.y > theAABB->minXYZ.y + vec_pPlankPositions[i].y)
				{
					if (pCurrentParticle->position.x < theAABB->maxXYZ.x + vec_pPlankPositions[i].x && pCurrentParticle->position.x > theAABB->minXYZ.x + vec_pPlankPositions[i].x)
					{
						if (pCurrentParticle->position.z< theAABB->maxXYZ.z+ vec_pPlankPositions[i].z&& !(pCurrentParticle->position.z< vec_pPlankPositions[i].x))
						{
							//std::cout << "2" << theAABB->maxXYZ.z+ vec_pPlankPositions[i].z<< " Y: " << pCurrentParticle->position.z<< std::endl;
							pCurrentParticle->position.z= theAABB->maxXYZ.z+ vec_pPlankPositions[i].x;
							acceleration.z = 0.0f;
						}
						else if (pCurrentParticle->position.z> theAABB->minXYZ.z+ vec_pPlankPositions[i].z&& !(pCurrentParticle->position.z> vec_pPlankPositions[i].x))
						{
							pCurrentParticle->position.z= theAABB->minXYZ.z+ vec_pPlankPositions[i].x;
							acceleration.z = 0.0f;
						}
					}
				}
				//if (pCurrentParticle->position.x < theAABB->minXYZ.x + vec_pPlankPositions[i].x)
				//{
				//	//std::cout << "3" << std::endl;
				//	pCurrentParticle->position.x = theAABB->minXYZ.x + vec_pPlankPositions[i].x;
				//}
				//if (pCurrentParticle->position.x > theAABB->maxXYZ.x + vec_pPlankPositions[i].x)
				//{
				//	//std::cout << "4" << std::endl;
				//	pCurrentParticle->position.x = theAABB->maxXYZ.x + vec_pPlankPositions[i].x;
				//}
				//if (pCurrentParticle->position.z < theAABB->minXYZ.z + vec_pPlankPositions[i].z)
				//{
				//	//std::cout << "5" << std::endl;
				//	pCurrentParticle->position.z = theAABB->minXYZ.z + vec_pPlankPositions[i].z;
				//}
				//if (pCurrentParticle->position.z > theAABB->maxXYZ.z + vec_pPlankPositions[i].z)
				//{
				//	//std::cout << "6" << std::endl;
				//	pCurrentParticle->position.z = theAABB->maxXYZ.z + vec_pPlankPositions[i].z;
				//}
			}
		}
	}
...
void cSoftBodyVerlet::SatisfyConstraints(int numOfIterations)
{
	const unsigned int NUM_ITERATIONS = numOfIterations;  
...
glm::vec3 cSoftBodyVerlet::getCenterBetween2Particles(int particleA, int particleB)
{
	return (vec_pParticles[particleA]->position + vec_pParticles[particleB]->position) / 2.0f;
}

cSoftBodyVerlet::sParticle* cSoftBodyVerlet::getHighestXParticle(void)
{
	sParticle* temp = vec_pParticles[0];
	for (int i = 0; i < vec_pParticles.size(); i++)
	{
		if (temp->position.x > vec_pParticles[i]->position.x) 
		{
			temp = vec_pParticles[i];
		}
	}
	return temp;
}
cSoftBodyVerlet::sParticle* cSoftBodyVerlet::getLowestXParticle(void)
{
	sParticle* temp = vec_pParticles[0];
	for (int i = 0; i < vec_pParticles.size(); i++)
	{
...
cSoftBodyVerlet::sParticle* cSoftBodyVerlet::getHighestZParticle(void)
{
	sParticle* temp = vec_pParticles[0];
	for (int i = 0; i < vec_pParticles.size(); i++)
	{
		if (temp->position.z > vec_pParticles[i]->position.z)
		{
			temp = vec_pParticles[i];
		}
	}
...
cSoftBodyVerlet::sParticle* cSoftBodyVerlet::getLowestZParticle(void)
{
	sParticle* temp = vec_pParticles[0];
	for (int i = 0; i < vec_pParticles.size(); i++)
	{
...
void cSoftBodyVerlet::CreateRandomBracing(unsigned int numberOfBraces,
											float minDistanceBetweenVertices, 
											float maxDistanceBetweenVertices)
{
	for ( unsigned int count = 0; count != numberOfBraces; count++ )
	{
		// Assume the distance is OK
		...
		do 
		{
			...
			// Distance OK?
			if ( distBetween < minDistanceBetweenVertices || distBetween > maxDistanceBetweenVertices)
			{
				// No
				bKeepLookingForParticles = true;
			}
