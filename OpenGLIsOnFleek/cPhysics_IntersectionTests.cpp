...
bool cPhysics::m_Sphere_Sphere_IntersectionTest(sPhsyicsProperties* pSphereA, sPhsyicsProperties* pSphereB)
{
	...
	if (!pSphereB->pTheAssociatedMesh)
	{
		return false;
	}
	
	if (!pSphereA->pTheAssociatedMesh)
	{
		return false;
	}

	glm::vec3 distanceVec = pSphereA->position - pSphereB->position;
	float distance = glm::dot(distanceVec, distanceVec);

	sPhsyicsProperties::sSphere* pSphereOne = (sPhsyicsProperties::sSphere*)(pSphereA->pShape);
	sPhsyicsProperties::sSphere* pSphereTwo = (sPhsyicsProperties::sSphere*)(pSphereB->pShape);


	float radiusSum = pSphereOne->radius + pSphereTwo->radius;

	if (distance <= radiusSum * radiusSum)
	{
		// normalized collision vector
		glm::vec3 collisionNormal = (glm::normalize(pSphereA->position - pSphereB->position));
		float restitution = 1.0f;


		float separatingVelocity = glm::dot(pSphereA->velocity - pSphereB->velocity, collisionNormal);

		if (separatingVelocity <= 0)
		{
			float newSepVelocity = -separatingVelocity * restitution;
			float delatVelocity = newSepVelocity - separatingVelocity;

			float totalInverseMass = pSphereA->inverse_mass + pSphereB->inverse_mass;
			if (totalInverseMass > 0)
			{
				float impulse = delatVelocity / totalInverseMass;

				glm::vec3 impulsePerIMass = collisionNormal * impulse;


				//pSphereA->velocity += impulsePerIMass * pSphereA->inverse_mass;
				//pSphereB->velocity += impulsePerIMass * pSphereB->inverse_mass;

				glm::vec3 tempVel = pSphereA->velocity;
				pSphereA->velocity = pSphereB->velocity;
				pSphereB->velocity = tempVel;
			}
		}

		sCollisionEvent theCollision;
		theCollision.pObjectA = pSphereA;
...
bool cPhysics::m_Sphere_AABB_IntersectionTest(sPhsyicsProperties* pSphere, sPhsyicsProperties* pAABB)
{
	...
	//// Does the physics object have a mesh object associated? 
	//if (!pSphere->pTheAssociatedMesh)
	//{
	//	return false;
	//}
	//
	//if (!pAABB->pTheAssociatedMesh)
	//{
	//	return false;
	//}
	...
	if (pSphere->position.y + pSphere_Details->radius < pAABB_Detail->minXYZ.y + pAABB->position.y) 
	{
		return false;
	}
	if (pSphere->position.y - pSphere_Details->radius > pAABB_Detail->maxXYZ.y + pAABB->position.y) 
	{
		return false;
	}
	if (pSphere->position.x + pSphere_Details->radius < pAABB_Detail->minXYZ.x + pAABB->position.x) 
	{
		return false;
	}
	if (pSphere->position.x - pSphere_Details->radius > pAABB_Detail->maxXYZ.x + pAABB->position.x) 
	{
		return false;
	}
	if (pSphere->position.z + pSphere_Details->radius < pAABB_Detail->minXYZ.z + pAABB->position.z) 
	{
		return false;
	}
	if (pSphere->position.z - pSphere_Details->radius > pAABB_Detail->maxXYZ.z + pAABB->position.z) 
	{
		return false;
	}
