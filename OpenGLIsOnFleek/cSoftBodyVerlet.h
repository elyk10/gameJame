...
public:
	...
	bool CreateSoftBodyBridge(sModelDrawInfo ModelInfo, glm::mat4 matInitalTransform = glm::mat4(1.0f));
	...
	void SatisfyConstraints(int numOfIterations = 10);
	...
	std::vector<glm::vec3> vec_pPlankPositions; 
	sPhsyicsProperties::sAABB* theAABB = new sPhsyicsProperties::sAABB(glm::vec3(-4.0f, -0.5f, -10.0f), glm::vec3(4.0f, 0.5f, 10.0f));
	...
	glm::vec3 getCenterBetween2Particles(int particleA, int particleB);

	sParticle* getHighestXParticle(void);
	sParticle* getLowestXParticle(void);
	sParticle* getHighestZParticle(void);
	sParticle* getLowestZParticle(void);
