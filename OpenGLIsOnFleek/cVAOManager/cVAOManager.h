...
public:
	...
	bool LoadModelIntoVAOWithManualBone(std::string fileName,
						  sModelDrawInfo &drawInfo, 
						  unsigned int shaderProgramID,
						  bool bIsDynamicBuffer = false); 
	
	bool LoadModelIntoVAOWithBones(std::string fileName,
						  sModelDrawInfo &drawInfo, 
						  unsigned int shaderProgramID,
						  bool bIsDynamicBuffer = false); 
	...
	bool LoadModelWithAssimpShadow(std::string fileName,
		sModelDrawInfo& drawInfo,
		unsigned int shaderProgramID, unsigned int shadowShader,
		bool bIsDynamicBuffer = false);

	bool LoadModelWithAssimpMultipleAnimations(std::string fileName,
		std::vector<std::string> animationFiles, 
		sModelDrawInfo& drawInfo,
		unsigned int shaderProgramID, unsigned int shadowShader, 
		bool bIsDynamicBuffer = false);

	bool LoadAnimationFile(std::string fileName,
		sModelDrawInfo& drawInfo);
...
private:
	...
	bool m_LoadTheFile_Ply_XYZ_N_RGBA_UV_ManualBone(std::string theFileName, sModelDrawInfo& drawInfo);
