...
bool cFBO::shutdown(void)
{
	...
	glDeleteTextures( 1, &(this->colourBrightnessTexture_1_ID) );
...
//************************************************************
	// Create the colour Brightness buffer (texture)
	glGenTextures(1, &(this->colourBrightnessTexture_1_ID) );		//g_FBO_colourTexture
	glBindTexture(GL_TEXTURE_2D, this->colourBrightnessTexture_1_ID);
...
// ***************************************************************
	...
	glFramebufferTexture(GL_FRAMEBUFFER,
						 ...
						 this->colourBrightnessTexture_1_ID, 0);
...
bool cFBO::initShadowFBO(int width, int height, std::string& error)
{
	this->width = width;
	this->height = height;
...
//	glCreateFramebuffers(1, &( this->ID ) );	// GL 4.5		//g_FBO
	...
	glBindFramebuffer(GL_FRAMEBUFFER, this->ID);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, this->colourTexture_0_ID, 0);
