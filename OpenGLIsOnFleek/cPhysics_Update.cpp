			...
			case sPhsyicsProperties::SPHERE:
				...
				case sPhsyicsProperties::AABB:
					...
					if (this->m_Sphere_AABB_IntersectionTest(pObjectA, pObjectB))
					{
						...
						sPhsyicsProperties::sSphere* pSphere_Detail = (sPhsyicsProperties::sSphere*)(pObjectA->pShape);
						
						if (pObjectB->friendlyName == "ground")
						{
							pObjectA->position.y = pAABB_Detail->maxXYZ.y + pSphere_Detail->radius;
						}
						else
						{
	...
	for (sPhsyicsProperties* pObject : this->m_vec_pPhysicalProps)
	{
		if (pObject->pTheAssociatedMesh)
		{
			if (thirdPersonView)
			{
				...
				{
					glm::vec3 cameraEyeDis = ::g_cameraEye - ::g_cameraTarget;
					::g_cameraEye = pObject->position + cameraEyeDis;
					::g_cameraTarget = pObject->position;
				}
			}
			pObject->pTheAssociatedMesh->setDrawPosition(pObject->position + pObject->meshOffset);
